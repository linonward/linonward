  ...
  return {
    ...
    acceptanceCriteria: draft.acceptanceCriteria.map((criterion): AcceptanceCriterion => {
      ...
      return {
        ...
        ...(previous?.evidence ? { evidence: previous.evidence } : {}),
      };
    }),
    steps: draft.steps.map((step): PlanStep => {
      const completed = completedSteps.get(step.id);
      return completed ?? { ...step, status: "pending", evidence: [] };
    }),
  };
}
```

Planner 返回修订草案后，`reconcilePlan` 保持 Goal 原文和仍存在的已完成 Step，保留它们的 evidence，并让 Plan `version + 1`。Harness 随后把变更原因写入 `planHistory` 并发出 `plan_revised` 事件。新计划仍要通过与初始计划相同的结构和依赖校验。
...
这六个测试分别验证依赖调度、互斥执行、证据门禁、重规划触发、状态协调和任务完成门禁。它们不调用真实模型或真实工具，因此结果稳定，也能在 Agent Loop 接入模型之前定位 Harness 的状态错误。
...
在 `agent-from-scratch` 目录运行命令。终端应先显示 `tests/plan.test.ts`、`6 tests passed`，再打印真实任务的 `TaskPlan`，并以退出码 0 结束。用上面六条清单审查实际输出；任何一条不满足，本章都不能算通过。
