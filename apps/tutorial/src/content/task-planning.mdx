...
单元测试通过后，再使用第 00 章写入 `GOAL.md` 的真实工程任务。暂时把规划入口接到 `src/index.ts`：这里会调用真实模型，但只生成并打印计划，不会修改文件或执行命令。
...
4. 修改 Step 必须依赖调查 Step，验证 Step 必须依赖修改和测试 Step；
...
这一步验证的是 Agent 对真实任务的规划能力：目标是否保持、步骤是否可执行、依赖是否合理、证据是否可检查。它还没有验证文件修改和命令执行；教程会在工具系统与 Agent Loop 完成后，用同一个任务做端到端验收。
