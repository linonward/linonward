<LessonOverview
  ...
  prerequisites="第 06 章已经有 ToolRegistry、统一执行入口和 function_call_output 回传链路。"
...
`SkillSummary` 中的路径供 Harness 定位文件，不需要发给模型。进入初始 Prompt 的只有经过长度限制和 XML 转义后的 `name` 与 `description`。
...
export async function loadSkillResource(input: {
  ...
  if (!active.loadedResources.includes(input.resourcePath)) {
    active.loadedResources.push(input.resourcePath);
  }
...
`scripts/` 被这个读取工具明确拒绝。脚本路径只有在 Skill 指令已经加载、用户任务确实需要且权限策略允许时，才能交给后续章节的命令执行工具；永远不要把“读取脚本”和“执行脚本”合并成一个隐式动作。
