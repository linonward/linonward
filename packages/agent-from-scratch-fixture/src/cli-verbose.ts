...
function eventDetail(event: AgentLoopEvent): string {
  switch (event.type) {
    ...
    case "plan_revised":
      return `version=${event.version} reason=${event.reason}`;
    case "run_stopped":
...
function observationOk(content: string): boolean | undefined {
  let parsed: unknown;
  try {
...
function toolObservationLines(state: AgentState): string[] {
  ...
  const lines = recent.map((source) => {
    const ok = observationOk(source.content);
    const fields = [
      `[tool] ${source.label}`,
      `callId=${source.id}`,
      ok === undefined ? "ok=unknown" : `ok=${String(ok)}`,
